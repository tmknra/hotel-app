...
class HotelController {
    ...
    def index() {
        ...
        if (params.entityPatternSearchInput == null && params.country == null) {
            ...
            /*
                Не получилось найти методы firstResult() и maxResults(). Вероятно, дело в версии gorm, как и было в
                ситуации ниже с методом eq(), перегрузки которого с параметром 'ignoreCase' нет в классе Criteria.
                Перепробовал возможные версии, несколько раз пересобирал билд целиком, но данных методов в классе
                Criteria нет. Возможно, что-то не так делаю. Могу только явно передать параметры в метод list(),
                что работает корректно - из базы возвращается список, ограниченный по количеству.
            * */
